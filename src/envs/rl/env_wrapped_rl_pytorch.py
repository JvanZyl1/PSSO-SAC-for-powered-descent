...
class GymnasiumWrapperPyTorch:
    ...
    def augment_action(self, action): # child
        return action
    
    def augment_state(self, state): # child
        return state
    ...
    def step(self, action):
        ...
        action_np = self.augment_action(action_np)
        if action_np.ndim == 2:
            action_np = action_np[0]
            
        state, reward, done, truncated, info = self.env.step(action_np)
        processed_state = self._process_state(state)
...
class rl_wrapped_env_pytorch(GymnasiumWrapperPyTorch):
    def __init__(self,
